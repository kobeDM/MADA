...
int main(int argc, char *argv[]){
  ...
  ViewWin->cd();
  pad0->Draw();  pad1->Draw();  pad2->Draw();  pad3->Draw();  pad4->Draw();
  ...
  while(iwaki.isAlive() && !end_flag){
	...
	if(event_data > 16){
	  unsigned evt_cnt 
	    = ((evt_data.at( 4) & 0xff) << 24) 
	    | ((evt_data.at( 5) & 0xff) << 16)
	    | ((evt_data.at( 6) & 0xff) <<  8) 
	    |  (evt_data.at( 7) & 0xff);
	  unsigned clk_cnt 
	    = ((evt_data.at( 8) & 0xff) << 24) 
	    | ((evt_data.at( 9) & 0xff) << 16)
	    | ((evt_data.at(10) & 0xff) <<  8) 
	    |  (evt_data.at(11) & 0xff);
	  unsigned trg_cnt 
	    = ((evt_data.at(12) & 0xff) << 24) 
	    | ((evt_data.at(13) & 0xff) << 16)
	    | ((evt_data.at(14) & 0xff) <<  8) 
	    |  (evt_data.at(15) & 0xff);
	  
	  for(int i=0; i<4; i++)
	    cout<<hex << setw(2) << setfill('0') << (evt_data.at(i) & 0xff) << " ";
	  cout <<dec;
	  cout <<endl<< "trig counter"<<setw(8) << setfill('0') << evt_cnt << " "
	       << endl<< "clock counter" << setw(8) << setfill('0') << clk_cnt << " "
	       <<	    endl<<"signal counter"  << setw(8) << setfill('0') << trg_cnt << endl;
	  ...
	  if(evt_data.size() > 20){
		...
		unsigned fadc
		  = ((evt_data.at(i*8+ch*2+16) & 0xff) << 8)
		  |  (evt_data.at(i*8+ch*2+17) & 0xff);
	    ...
	    int offset = 8212;
	    unsigned flag
	      = ((evt_data.at(offset+0) & 0xff) << 24)
	      | ((evt_data.at(offset+1) & 0xff) << 16)
	      | ((evt_data.at(offset+2) & 0xff) <<  8)
	      |  (evt_data.at(offset+3) & 0xff);
	    while(flag != 0x75504943){
	      int clk 
		= ((evt_data.at(offset+2) & 0x07) << 8)
		|  (evt_data.at(offset+3) & 0xff);
	      for(int i=0; i<16; i++){
		for(int j=0; j<8; j++){
	  ...
	  event_data = 0;
	  evt_data.clear();
	  pad0->cd();  image->Draw("col");
	  pad1->cd();  wave[0]->Draw("p");
	  pad2->cd();  wave[1]->Draw("p");
	  pad3->cd();  wave[2]->Draw("p");
	  pad4->cd();  wave[3]->Draw("p");
